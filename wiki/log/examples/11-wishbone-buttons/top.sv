...
logic [31:0] read_led0;
logic [31:0] read_led1;

always_ff @( posedge clk ) begin
    
    case ({SW1, SW2})
        2'b00: begin //-- Read port 0
            mem_bus.we <= 0;
            mem_bus.adr = 32'h0008_0000;
            if (mem_bus.ack)
                read_led0 <= mem_bus.dat_miso;
        end
        2'b10: begin //-- Write port 0 (increment by 1)
            mem_bus.we <= 1;
            mem_bus.adr = 32'h0008_0000;
            mem_bus.dat_mosi <= read_led0 + 1;
        end
        2'b01: begin //-- Read port 1
            mem_bus.we <= 0;
            mem_bus.adr = 32'h0008_0001;
            if (mem_bus.ack)
                read_led1 <= mem_bus.dat_miso;
        end
        2'b11: begin //-- Write port 1 (increment by 2)
            mem_bus.we <= 1;
            mem_bus.adr = 32'h0008_0001;
            mem_bus.dat_mosi <= read_led1 + 2;
        end
        default: begin
            mem_bus.we <= 0;
            mem_bus.adr = 32'h0008_0001;
